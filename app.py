...
def student_questions(topic_id):
    ...
    cur.execute("SELECT question_id, question_text FROM questions WHERE topic_id = %s", (topic_id,))
    questions = cur.fetchall()
    return render_template('student_questions.html', topic_id=topic_id, questions=questions)
...
def ask():
    ...
    # Map roles correctly for Gemini
    formatted_messages = []
    for h in history:
    ...
    try:
        prompted_messages = f"Write the following in latex without document.\n{formatted_messages}"
        response = model.generate_content(prompted_messages)
        text_response = model.generate_content(response.text)
        return jsonify({'reply': text_response.text})
    except Exception as e:
